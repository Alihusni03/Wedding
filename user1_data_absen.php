      ...
      <div class="main-content">
        ...
        <div class="section">
          <div class="dashboard-box">
            <h3>Data Daftar Hadir Tamu</h3>
            <div class="box">
              <p><a href="user1_tambah_absen.php">Tambah Data</a></p>
              <table border="1" cellspacing="0" class="table">
                ...
                <tbody>
                  <?php
                    $no = 1;
                    $absen = mysqli_query($conn, "SELECT * FROM tb_absen LEFT JOIN tb_mempelai USING (id_mempelai) ORDER BY id_absen DESC");
                    if(mysqli_num_rows($absen) > 0){
                    while($row = mysqli_fetch_array($absen)){
                  ?>
                  <tr>
                    <td><?php echo $no++ ?></td> 
                    <td><?php echo $row['nama'] ?></td>
                    <td><?php echo $row['mempelai'] ?></td>
                    <td><?php echo $row['status'] ?></td>
                    <td>
                      <a href="user1_proses_hapus.php?ida=<?php echo $row['id_absen'] ?>" onclick="return confirm('Yakin ingin Hapus ?')">hapus</a>
                    </td>
                  </tr>
                  <?php }}else{ ?>
                      <tr>
