      ...
      <div class="main-content">
        ...
        <div class="section">
          <div class="dashboard-box">
            <h3>Data Lokasi</h3>
            <div class="box">
              <p><a href="user1_tambah_lokasi.php">Tambah Data</a></p>
              <table border="1" cellspacing="0" class="table">
                <thead>
                  <tr>
                    <th width="60px">No</th>
                    <th>Denah Akad Nikah</th>
                    <th>Denah Resepsi</th>
                    <th>status</th>
                ...
                <tbody>
                  <?php
                    $no = 1;
                    $lokasi = mysqli_query($conn, "SELECT * FROM tb_lokasi LEFT JOIN tb_mempelai USING (id_mempelai) ORDER BY id_lokasi DESC");
                    if(mysqli_num_rows($lokasi) > 0){
                    while($row = mysqli_fetch_array($lokasi)){
                  ?>
                  <tr>
                    <td><?php echo $no++ ?></td>
                    <td><?php echo $row['denah_akad'] ?></td>
                    <td><?php echo $row['denah_resepsi'] ?></td>
                    <td><?php echo $row['status'] ?></td>
                    <td>
                      <a href="user1_edit_data_lokasi.php?id=<?php echo $row['id_lokasi'] ?>">edit</a> || <a href="user1_proses_hapus.php?idl=<?php echo $row['id_lokasi'] ?>" onclick="return confirm('Yakin ingin Hapus ?')">hapus</a>
                    </td>
                  </tr>
                  <?php }}else{ ?>
                      <tr>
